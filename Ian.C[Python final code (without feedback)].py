...
##Function to find how soon student debt will be paid back with minimum repayments   
...
debt_paid_list = []
int_year_list = []
#total_paid_list = [] #Not implimented yet 
#total_int_list =[]    #
def time_to_repay(option, debt, salary):
    ...
    global salary_list
    global debt_list
    global debt_paid_list 
    global void_year 
    #global total_paid_list
    ...
    while debt > 0:
        ...
        debt_paid_list.append(round(rate_repay*(salary - threshold) + overpayments))
        ...
        if count >= void_year:
            ...
            st.write(f"""
                     ...
                     from salary deductions. You will pay back £{sum(debt_paid_list)} before your loan
    ...
    #
    if not debt_voided:
        debt_paid = accu_int + initial_debt + overpayments
        st.write(f"""
        ...
        if overpayments != 0:
            st.write(f"""
                     ...
                     You will pay back £{round(debt_paid)} in total, meaning you will pay £{round(accu_int)}
    ...
    if salary >= threshold:
        debt_paid_list.pop()
        debt_paid_list.append(debt_list[-1] + int_year_list[-1])
...
def investment_func(yearly_pay_in,r,years_elapsed,year_start):
    #Globals
    #
    global year_counter
...
salary_breakdown()                    # Application of functions
...
df = pd.DataFrame(
    {
     ...
     'paid This Year': [f"{p}" for p in debt_paid_list],
     'Interest This Year': [f"{t}" for t in int_year_list]
     #'Total paid': [f"{tp}" for tp in total_paid_list],
     #'Total Interest': [f"{ti}" for ti in total_int_list]
     },
     columns=['Year', 'salary', 'Debt', 'Int. Rate %', 'paid This Year', 'Interest This Year' ]
)
...
df2 = pd.DataFrame(
    {
     ...
     columns=['Year', 'Debt', 'paid This Year']
)
...
#
debt_paid = accu_int + initial_debt
labels = 'Initial Debt', 'Total Interest'
sizes = [initial_debt/debt_paid, accu_int/debt_paid]
...
###-"Index Funds"-------------------------------------------------------------------------------------------------
investment_func(yearly_pay_in,r,years_elapsed,year_start_compound)
...
df3 = pd.DataFrame(
    {
     ...
     'Investment After Debt repaid':[f"{b}" for b in accumulated_amount_option2_list],
     },
    columns=['Year', 'Early Investment', 'Investment After Debt repaid']
)
