...
def time_to_repay(option, debt, salary):
    ...
    while debt > 0:
        ...
        if count >= 30:
            ...
            st.write(f"""
                     ...
                     after graduation. The remaining years until this date have been used in the
    ...
    if not debt_voided:
        ...
        st.write(f"""
                 You will manage to pay off your student loan after year {count} from salary deductions alone.
...
st.sidebar.subheader("Additional Plots with info")
...
###Unfinished code ---------------------------------------------------------------------------------------------
##Index funds -> First draft - works in python but not in web app
#compound interest
#p = stock
#r = 8
#t = 1
...
#def compound_interest(p,r,t):
    #global count
    #while count < 40:
        #amount.append(p*((1+r/100)**t))
        #p += stock
        #count += 1
        #year_list_2.append(current_year + count)
    #ci = amount[-1]-p
#compound_interest(p, r, t)
#df3 = pd.DataFrame(
    #{
     #'Year': [f"{y}" for y in year_list_2],
     #'amount': [f"{a}" for a in amount],
     #},
     #columns=['Year', 'Amount']
#)
...
###-Next steps ------------------------------------------------------------------------------------------------
# 1. Fix existing code -> Done 
# 2. Add in options for investment ->  index fund pay ins 
# 3. Find a pleasing format -> I like the linegraph and pie chart combo but the linegraph is too small...etc
# Also. Time to pay function currently uses anual repayments rarther than monthly -> this isn't too bad but
#...    causes discrepencies when dealing with overpayments so mathematically incorrect. However,
#...    this isn't a maths degree - code works well so this "fix" is lowest on the list.  
# On a similar note I havent considered inflation. 
